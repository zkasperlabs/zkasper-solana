//! BN254 PLONK, through Solana's `alt_bn128` syscalls.
//!
//! Zisk wraps a VADCOP final proof into a snarkjs PLONK proof — 768 bytes,
//! `uint256[24]`, one public input. This module is a line-for-line
//! transliteration of the verifier Zisk ships as
//! `zisk-contracts/PlonkVerifier.sol`: where the Solidity reaches for a
//! precompile this reaches for the matching syscall, and where it uses `mulmod`
//! this uses [`ark_bn254::Fr`]. Nothing here reimplements curve arithmetic.
//!
//! The circuit constants it verifies against live in [`vk`], which is the one
//! place in the tree that names a Zisk release.
...
const FAILED: ZkasperError = ZkasperError::ProofVerificationFailed;
...
fn g1_mul(point: &G1, scalar: &Fr) -> R<G1> {
    let mut input = [0u8; 96];
    input[..64].copy_from_slice(point);
    input[64..].copy_from_slice(&fr_bytes(scalar));
    let out = alt_bn128_g1_multiplication_be(&input).map_err(|_| FAILED)?;
    let mut r = [0u8; 64];
    r.copy_from_slice(&out);
    Ok(r)
}

fn g1_add(a: &G1, b: &G1) -> R<G1> {
    let mut input = [0u8; 128];
    input[..64].copy_from_slice(a);
    input[64..].copy_from_slice(b);
    let out = alt_bn128_g1_addition_be(&input).map_err(|_| FAILED)?;
    let mut r = [0u8; 64];
    r.copy_from_slice(&out);
    Ok(r)
}
...
fn g1_mul_acc(acc: &G1, point: &G1, scalar: &Fr) -> R<G1> {
...
impl<'a> Proof<'a> {
    /// Split the 24 words, rejecting any evaluation that is not below `r`.
    ///
    /// The range check is not redundant the way the curve check below is: the
    /// transcript hashes the wire bytes, so a non-canonical evaluation would
    /// hash differently from the value the algebra uses.
    ...
    /// `checkProofData`, which the verification path does **not** run.
    ///
    /// Solidity has to test `y^2 == x^3 + 3` itself because `ecAdd` and `ecMul`
    /// are cheap enough to be worth guarding. On Solana every one of the nine
    /// commitments is an operand to an `alt_bn128` syscall before it can reach
    /// the pairing, and the syscall parses each coordinate canonically and
    ...
    ///
    /// Kept public so `zkasper-plonk-cost` can price it and so
    /// `program-tests` can show, commitment by commitment, that a corrupted
    /// point is rejected without it.
...
/// The single public input Zisk's wrap commits:
/// `sha256(programVK || publicValues || rootCVadcopFinal) mod r`.
///
/// `program_vk` names the guest whose execution the VADCOP recursion attested,
/// and `rootCVadcopFinal` names the STARK verification key that recursion
/// terminated at. Both belong to the *statement*, not to the proof, so both are
/// held by this program and neither is ever read from a submission — see the
/// module docs on [`crate::state::LightClientState::program_vk`] and [`vk`].
pub fn public_input(program_vk: &ProgramVk, public_values: &[u8; vk::PUBLIC_VALUES_LEN]) -> Fr {
    let mut preimage = [0u8; 32 + vk::PUBLIC_VALUES_LEN + 32];
    preimage[..32].copy_from_slice(program_vk);
    preimage[32..32 + vk::PUBLIC_VALUES_LEN].copy_from_slice(public_values);
    preimage[32 + vk::PUBLIC_VALUES_LEN..].copy_from_slice(&vk::ROOT_C_VADCOP_FINAL);
...
fn lagrange_1(c: &Challenges) -> R<Fr> {
...
fn calculate_d(proof: &Proof, c: &Challenges, eval_l1: &Fr) -> R<G1> {
    let mut d = vk::QC;
...
fn calculate_f(proof: &Proof, c: &Challenges, d: &G1) -> R<G1> {
...
fn calculate_e(proof: &Proof, c: &Challenges, r0: &Fr) -> R<G1> {
    let s = -*r0
        + proof.eval_a * c.v[0]
        + proof.eval_b * c.v[1]
        + proof.eval_c * c.v[2]
...
fn check_pairing(proof: &Proof, c: &Challenges, f: &G1, e: &G1) -> R<()> {
    ...
    let b1 = g1_add(
        ...
        &g1_mul(
            &proof.wxiw,
            &(c.u * c.xi * fr_canonical(&vk::W1).ok_or(FAILED)?),
        )?,
    )?;
    let b1 = g1_add(&b1, f)?;
    ...
    match alt_bn128_pairing_be(&input) {
        Ok(out) if out.last() == Some(&1) => Ok(()),
        _ => Err(FAILED),
    }
}

/// Verify a wrapped Zisk proof of `public_values`, produced by `program_vk`.
pub fn verify(
    proof_bytes: &[u8],
    program_vk: &ProgramVk,
    public_values: &[u8; vk::PUBLIC_VALUES_LEN],
) -> R<()> {
    verify_with(proof_bytes, program_vk, public_values, false)
}

/// The same, with [`Proof::well_formed`] optionally run first.
///
/// The flag exists so `zkasper-plonk-cost` can price the check and so the tests
/// can compare the two paths on the same inputs. The program always passes
/// `false`; see [`Proof::well_formed`] for why that is sound.
pub fn verify_with(
    proof_bytes: &[u8],
    program_vk: &ProgramVk,
    public_values: &[u8; vk::PUBLIC_VALUES_LEN],
    ...
    let proof = Proof::parse(proof_bytes).ok_or(FAILED)?;
    if check_membership && !proof.well_formed() {
        return Err(FAILED);
    }
    let pi = public_input(program_vk, public_values);
